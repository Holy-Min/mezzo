...
//    }
    ...
    public AdManView requestMovie(final Button btn, final Context c, final int p, final int m, final int s, final String mode, final RelativeLayout bannerArea, final int w, final int h, String autoPlay, final String autoReplay, String clickFullArea, String muted, String soundBtnShow, String clickBtnShow, String skipBtnShow, String showClose) {
        ...
        adData.isPermission(MainActivity.isLocation, MainActivity.isReadPhone);
        adData.setRequestmode(mode);
        final AdManView movieView = new AdManView(c);
        movieView.setData(adData, new AdListener() {
            @Override
            public void onAdSuccessCode(Object obj, String id, final String type, final String status, String jsonDataString) {
                if(btn!= null) {
                    btn.setEnabled(true);
                }
                Navimanager.getInstance().onAdSuccessCode(type, status, jsonDataString,  new Handler() {
                    ...
                    public void dispatchMessage(Message msg) {
                        ...
                        long successTime = Calendar.getInstance().getTimeInMillis();
                        log += "\r\ndiff: "+(currentTime - successTime);
            ...
            @Override
            public void onAdFailCode(Object obj, String id, String type, String status, String jsonDataString) {
                if(btn!= null) {
                    btn.setEnabled(true);
                }
                if(movieView!=null){
            ...
            @Override
            public void onAdErrorCode(Object obj, String id, String type, String status, String failingUrl) {
                if(btn!= null) {
                    btn.setEnabled(true);
                }
                if(movieView!=null){
            ...
            @Override
            public void onAdEvent(Object obj, String id, String type, String status, String jsonDataString) {
                if(btn!= null) {
                    btn.setEnabled(true);
                }
            ...
            @Override
            public void onPermissionSetting(Object obj, String id) {
                showSettingsAlert(c);
            }
        });
        movieView.request();
    ...
    public void requestBanner(final Button btn, final Context c, final int p, final int m, final int s, final String mode, final String IsFloatingBanner, final ViewGroup bannerArea) {
        isBannerInit = false;
        btn.setEnabled(false);
        ...
        adData.isPermission(MainActivity.isLocation, MainActivity.isReadPhone);
        adData.setRequestmode(mode);
    //   adData.setIsFloatingBanner(IsFloatingBanner);
        bannerView = new AdManView(c);
        bannerView.setData(adData, new AdListener() {
            @Override
            public void onAdSuccessCode(Object obj, String id, final String type, final String status, final String jsonDataString) {
                btn.setEnabled(true);
                Navimanager.getInstance().onAdSuccessCode(type, status, jsonDataString, new Handler() {
                    @Override
                    public void dispatchMessage(Message msg) {
    ...
    public void requestInlayoutInter(final Button btn, final Context c, final int p, final int m, final int s, final String mode,  final RelativeLayout bannerArea) {
        btn.setEnabled(false);
        final long currentTime = Calendar.getInstance().getTimeInMillis();
        bannerArea.removeAllViewsInLayout();
        AdData adData = new AdData();
        ...
        inlayoutInter.request();
    }


    public void requestInter(final Button btn, final Context c, final int p, final int m, final int s, final String mode, final String isPopup) {
        btn.setEnabled(false);
        final long currentTime = Calendar.getInstance().getTimeInMillis();
        AdData adData = new AdData();
        String appName = "appname";
        ...
        adManInterActivity.setData(adData, new AdListener() {
            @Override
            public void onAdSuccessCode(Object obj, String id, String type, String status, String jsonDataString) {
                btn.setEnabled(true);
                Navimanager.getInstance().onAdSuccessCode(type, status, jsonDataString, new Handler() {
                    @Override
                    public void dispatchMessage(Message msg) {
        ...
        adManInterActivity.request();
    }

    public void requestEnding(final Button btn, final Context c, final int p, final int m, final int s, final String mode, final int w, final int h) {
        btn.setEnabled(false);
        final long currentTime = Calendar.getInstance().getTimeInMillis();
        AdData adData = new AdData();
        String appName = "appname";
    ...
    public void onAdFailCode(String type, String status, String jsonDataString, Handler handler) {
        ...
        if(AdResponseCode.Status.NOAD.equals(status)){
            ...
            msg.obj = "onAdFailCode [ " + status + " ] " + "DEVICE_AD_INTERVAL";
            handler.dispatchMessage(msg);
        }
    }
    ...
    public void showEndingBanner(final Context c, final int w, final int h){
        ...
        if(adManEnding.getParent() != null){
            ((ViewGroup) adManEnding.getParent()).removeView(adManEnding);
        }
